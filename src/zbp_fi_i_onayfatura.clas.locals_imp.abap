CLASS lhc_zfi_i_onayfatura DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PUBLIC SECTION.
  ...
  PRIVATE SECTION.
    ...
    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zfi_i_onayfatura RESULT result.

    METHODS create FOR MODIFY
      IMPORTING entities FOR CREATE zfi_i_onayfatura.

    METHODS update FOR MODIFY
      IMPORTING entities FOR UPDATE zfi_i_onayfatura.

    METHODS delete FOR MODIFY
      IMPORTING keys FOR DELETE zfi_i_onayfatura.

    METHODS read FOR READ
      IMPORTING keys FOR READ zfi_i_onayfatura RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK zfi_i_onayfatura.

ENDCLASS.

CLASS lhc_zfi_i_onayfatura IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<fs_entity>).
      ...
      IF sy-subrc = 0.
        ...
        APPEND VALUE #( %key = <fs_entity>-%key ) TO failed-zfi_i_onayfatura.
        APPEND VALUE #(
          %key = <fs_entity>-%key
          %msg = new_message(
            id      = 'ZFI'
            ...
            v1      = |Aynı paymentorder ve paymentline zaten mevcut: { <fs_entity>-paymentorder }|
          )
        ) TO reported-zfi_i_onayfatura.
...
*       Doküman ekleme
            ...
            " Multipart form data oluştur (DÜZELTİLMİŞ)
            DATA(lv_form_header) =
              |--{ lv_boundary }{ lv_crlf }| &&
            ...
            " Headers ve body ayarla (SADECE BİNARY SET ET!)
      ...
      paymentline   = <fs_entity>-paymentline
...
*      DATA(ls_doc) = VALUE zfi_db_doc(
*        obj_id  = <lfs_doc>-obj_id
*        repoid  = <lfs_doc>-repoid
*        ).
*      APPEND ls_doc TO zcl_zfi_api_buffer=>mt_id_doc.
...
CLASS lsc_zfi_i_onayfatura DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zfi_i_onayfatura IMPLEMENTATION.
